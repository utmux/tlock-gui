...
- 开始批处理时通过 Windows 存储属性检测输入盘；同一机械硬盘包含多个任务且并发大于 1 时，默认建议本批次改为 1，并允许用户保留原设置。SSD/NVMe 默认仍为 2。
...
- 完整 stdout/stderr 日志；`stderr` 非空不等同失败。提前解密时会根据 quicknet 轮次显示预计可解密的本地日期时间、剩余时长以及目标/当前轮次；失败详情会直接出现在批次结束弹窗中，也保留在日志和状态提示中。
...
任务管理器的总百分比按全部逻辑处理器平均：例如 12 个逻辑处理器上的约 8% 已接近一个逻辑核心满载。TLockGUI 默认并行运行最多 2 个 `tle.exe`，可在高级设置中调整为 1–8。并发提高后 CPU 利用率通常会上升，但实际速度仍可能受磁盘或网络限制；机械硬盘建议使用 1，SSD 建议从 2 开始，根据吞吐和温度逐步调整。
...
- 默认 quicknet 的预计可解密时间按链的 3 秒周期换算；节点同步和网络传输可能带来数秒延迟，应以实际成功解密为准。
